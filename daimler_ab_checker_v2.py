#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Daimler AB 与 Bestellunterlagen 检查工具 v2。
...
def is_probable_code(token: str) -> bool:
    ...
    if not CODE_RE.fullmatch(token) or token in STOP_CODES: return False
    if token.isdigit(): return 2 <= len(token) <= 3 and not token.startswith("0")
    # Daimler Codes: normally contain a digit; local J codes can be 4-5 letters.
    return any(c.isdigit() for c in token) or bool(re.fullmatch(r"J[A-Z]{2,4}", token)) or token in {"ZSX"}
...
def extract_codes(page_texts: list[str], page_rows: list[list[str]], source: str) -> dict[str, CodeItem]:
    ...
    for page_no, rows in enumerate(page_rows[:limit], 1):
        ...
        for raw in rows:
            ...
            upper = line.upper()
            if page_no == 1 and any(a in upper for a in ("FAHRZEUGAUSSTATTUNG", "FAHRGESTELLAUSFUEHRUNG", "SONDERAUSSTATTUNG")):
                started = True; line = trim_line(line, source, page_no); upper = line.upper()
            if not started or not line: continue
            if any(x.lower() in line.lower() for x in HEADER_WORDS) and not any(a in upper for a in ("FAHRZEUGAUSSTATTUNG", "FAHRGESTELLAUSFUEHRUNG", "SONDERAUSSTATTUNG")): continue
            if any(x.lower() in line.lower() for x in END_WORDS): break
            if line.upper() in SECTION_TITLES: continue
            # A coordinate line normally contains one item. If OCR merged items, split at every valid Code.
            starts = [m for m in re.finditer(r"(?<![A-Z0-9])([A-Z0-9]{2,6})\s+", line, re.I) if is_probable_code(m.group(1))]
            if not starts: continue
            for i, m in enumerate(starts):
                code = normalize_code(m.group(1)); end = starts[i+1].start() if i+1 < len(starts) else len(line)
                desc = clean(line[m.end():end])
                if len(desc) < 2: continue
                if any(x in desc.upper() for x in ("REGISTERGERICHT","IBAN","UST-IDNR","ALBERT-ZIEGLER-STR","DATUM :")): continue
                optional = bool(re.match(r"^(OHNE|ENTFALL)\b", desc, re.I))
                item = CodeItem(code, desc[:600], page_no, source, optional)
                if code not in found or len(desc) > len(found[code].description): found[code] = item
    return found
...
def find_j_alternative(item, ab_codes, used):
    best=None; score=0.0
    for code,c in ab_codes.items():
        if code in used: continue
        v=similarity(item.description,c.description)
...
def write_outputs(base: Path,payload: dict):
    ...
    with pd.ExcelWriter(Path(str(base)+"_Codes.xlsx"),engine="openpyxl") as w:
        pd.DataFrame(rows).to_excel(w,index=False,sheet_name="Code Check"); pd.DataFrame(payload["field_checks"]).to_excel(w,index=False,sheet_name="Key Fields")
        ws=w.book["Code Check"]; ws.freeze_panes="A2"; ws.auto_filter.ref=ws.dimensions
        for c,v in {"A":18,"B":55,"C":16,"D":14,"E":14,"F":55,"G":12,"H":48,"I":55}.items(): ws.column_dimensions[c].width=v
